...
@app.get("/api/metrics/timeseries/confidence")
async def get_confidence_timeseries(bucket_seconds: int = 60, num_buckets: int = 20):
    """
    Get time-series data for cumulative average confidence.
    
    Returns cumulative average confidence scores up to each time point.
    This metric validates that semantic caching maintains response quality
    when reusing documents from similar queries.
    
    Args:
        bucket_seconds: Size of each time bucket in seconds (default: 60 = 1 minute)
        num_buckets: Number of time buckets to return (default: 20)
        
    WHY: Confidence is the key quality metric for validating the semantic
    caching strategy. High confidence scores for cached results prove that the system
    ...
    """
    try:
        return {"data": orchestrator.get_confidence_timeseries(bucket_seconds, num_buckets)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
